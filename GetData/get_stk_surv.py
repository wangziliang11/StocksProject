...
def get_stk_surv(ts_code):
    data = pro.stk_surv(ts_code=ts_code)
    return data

def updateData(ts_code,data_path):
    try:
        file_path = data_path + ts_code + '.csv'
        data = get_stk_surv(ts_code=ts_code)
...
if __name__ == '__main__':
    ...
    ##### 获取数据 #####
    data_path = '../Data/Dynamic/stk_surv/'
    mode = 'f'
    global count
    global start_time
    count = 0
    start_time = time.time()
    logger.info('开始更新 {} stk_surv 数据'.format(date.today().strftime("%Y%m%d")))
