...
def getLogging():
    ...
    test_log = logging.FileHandler('../logs/get_fina_mainbz.log', 'a', encoding='utf-8')
    # 向文件输出的日志级别
...
def updateData(ts_code,data_path):
    try:
        ...
        data = get_fina_mainbz(ts_code=ts_code)
...
if __name__ == '__main__':
    ...
    ##### 获取数据 #####
    data_path = '../Data/Dynamic/fina_mainbz/'
    mode = 'f'
    global count
    global start_time
    count = 0
    start_time = time.time()
    logger.info('开始更新 {} fina_mainbz 数据'.format(date.today().strftime("%Y%m%d")))
