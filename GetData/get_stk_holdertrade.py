...
def getLogging():
    ...
    test_log = logging.FileHandler('../logs/get_stk_holdertrade.log', 'a', encoding='utf-8')
    # 向文件输出的日志级别
...
def get_stk_holdertrade(ts_code):
    data = pro.stk_holdertrade(ts_code=ts_code)
    # 获取某一季度全部股票数据
...
def updateData(ts_code,data_path):
    try:
        ...
        data = get_stk_holdertrade(ts_code=ts_code)
...
if __name__ == '__main__':
    ...
    ##### 获取数据 #####
    data_path = '../Data/Dynamic/stk_holdertrade/'
    mode = 'f'
    global count
    global start_time
    count = 0
    start_time = time.time()
    logger.info('开始更新 {} stk_holdertrade 数据'.format(date.today().strftime("%Y%m%d")))
