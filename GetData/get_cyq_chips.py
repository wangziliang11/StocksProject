...
def getLogging():
    ...
    test_log = logging.FileHandler('../logs/get_cyq_chips.log', 'a', encoding='utf-8')
    # 向文件输出的日志级别
...
def get_cyq_chips(ts_code):
    data = pro.cyq_chips(ts_code=ts_code)
    return data

def updateData(ts_code,data_path):
    try:
        file_path = data_path + ts_code + '.csv'
        data = get_cyq_chips(ts_code=ts_code)
...
if __name__ == '__main__':
    ...
    ##### 获取数据 #####
    data_path = '../Data/Dynamic/cyq_chips/'
    mode = 'f'
    global count
    global start_time
    count = 0
    start_time = time.time()
    logger.info('开始更新 {} cyq_chips 数据'.format(date.today().strftime("%Y%m%d")))
