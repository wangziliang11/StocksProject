...
# sys.path.extend(['/Users/user/PycharmProjects/StocksProject', '/Users/user/PycharmProjects/StocksProject', '/Users/user/PycharmProjects/StocksProject'])
...
def getLogging():
    ...
    test_log = logging.FileHandler('../logs/get_weekly.log', 'a', encoding='utf-8')
    # 向文件输出的日志级别
...
def get_weekly(ts_code,start_date=(date.today()-timedelta(7)).strftime("%Y%m%d"),end_date=date.today().strftime("%Y%m%d")):
...
def updateData(ts_code,start_date=(date.today()-timedelta(7)).strftime("%Y%m%d"),end_date=date.today().strftime("%Y%m%d"),mode='s'):
    # model = s 增量更新
    #         f 全量更新
    try:
        if mode == 'f': a = int('a')
        df_hist = pd.read_csv('../Data/Dynamic/weekly/' + ts_code + '.csv')
        data = get_weekly(ts_code=ts_code, start_date=start_date, end_date=end_date)
        data['trade_date'] = pd.to_numeric(data['trade_date'])
        data = data.append(df_hist)
        # data['trade_date'].drop_duplicates()
        data = data.drop_duplicates(['trade_date'])
        data.set_index('trade_date', inplace=True)
        data.to_csv('../Data/Dynamic/weekly/' + ts_code+'.csv')
    except:
        logger.error('{0} 股票不存在,重新拉取全量数据!'.format(ts_code))
        data = get_weekly(ts_code=ts_code,start_date=(date.today() - timedelta(365*30)).strftime("%Y%m%d"))
        data.set_index('trade_date', inplace=True)
        data.to_csv('../Data/Dynamic/weekly/' + ts_code+'.csv')
...
if __name__ == '__main__':
    ...
    lastweek = (date.today() - timedelta(7)).strftime("%Y%m%d")
    ...
    for ts_code in ts_codes:
        updateData(ts_code, lastweek, end_date,mode='s')
        count += 1
        if count % 100 == 0:
