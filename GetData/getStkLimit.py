...
def getLogging():
    ...
    test_log = logging.FileHandler('../logs/get_stk_limit.log', 'a', encoding='utf-8')
    # 向文件输出的日志级别
...
def get_stk_limit(ts_code,trade_date,start_date,end_date):
    data = pro.stk_limit(ts_code=ts_code, trade_date=trade_date,start_date=start_date,end_date=end_date)
    return data
...
def updateData(ts_code,trade_date,start_date,end_date,data_path):
    try:
        ...
        if os.path.exists(file_path):
            ...
            data = get_stk_limit(ts_code=ts_code, trade_date=trade_date,start_date=start_date, end_date=end_date)
...
if __name__ == '__main__':
    ...
    ##### 获取数据 #####
    data_path = '../Data/Dynamic/stk_limit/'
    mode = 'f'
    global count
    global start_time
    count = 0
    start_time = time.time()
    logger.info('开始更新 {} stk_limit 数据'.format(date.today().strftime("%Y%m%d")))
