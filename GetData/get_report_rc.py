# 获取券商（卖方）每天研报的盈利预测数据，数据从2010年开始，每晚19~22点更新当日数据(8000积分可用)
...
def getLogging():
    ...
    test_log = logging.FileHandler('../logs/get_report_rc.log', 'a', encoding='utf-8')
    # 向文件输出的日志级别
...
def get_report_rc(ts_code):
    data = pro.report_rc(ts_code=ts_code)
    return data

def updateData(ts_code,data_path):
    try:
        file_path = data_path + ts_code + '.csv'
        data = get_report_rc(ts_code=ts_code)
...
if __name__ == '__main__':
    ...
    ##### 获取数据 #####
    data_path = '../Data/Dynamic/report_rc/'
    mode = 'f'
    global count
    global start_time
    count = 0
    start_time = time.time()
    logger.info('开始更新 {} report_rc 数据'.format(date.today().strftime("%Y%m%d")))
